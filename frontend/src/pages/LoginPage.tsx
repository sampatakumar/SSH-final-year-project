...
export default function LoginPage() {
  ...
  const features = [
    { icon: Bot, title: "Smart AI Mentor", desc: "Autonomous career guidance & role benchmarks" },
    { icon: GitBranch, title: "GitHub Intelligence", desc: "Automated repo health, README & quality audits" },
    { icon: GraduationCap, title: "EduTube Learning", desc: "Interactive programming video courses" },
    { icon: Target, title: "Skill Gap Analysis", desc: "Actionable roadmap tailored to your target job" },
    { icon: FileText, title: "Resume AI", desc: "Targeted ATS optimization and instant builds" },
    { icon: Globe, title: "Portfolio Generator", desc: "One-click deployment with custom domains" },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#070A12] text-foreground flex flex-col justify-center relative overflow-hidden selection:bg-indigo-500/30">
      ...
      <header className="absolute top-0 left-0 right-0 p-6 flex items-center justify-between max-w-7xl mx-auto z-20">
        ...
        >
          Don't have an account? <span className="text-indigo-400 font-semibold underline-offset-4 hover:underline">Create one</span>
        </Link>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-28 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        {/* Left Column: Product Storytelling (Desktop) */}
        <motion.div
        ...
        >
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs font-semibold mb-4">
              <Sparkles className="w-3.5 h-3.5" /> Developer Intelligence Platform
            </div>
            ...
            <p className="text-slate-400 text-sm mt-3 max-w-md leading-relaxed">
              Sign in to access your unified developer dashboard, resume engine, EduTube courses, and smart career mentor.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3.5 pt-2">
            {features.map((f, i) => (
              <div
                key={i}
                className="p-3.5 rounded-xl bg-slate-900/60 border border-white/[0.08] backdrop-blur-md flex items-start gap-3 shadow-sm hover:border-indigo-500/30 transition-colors"
              >
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 leading-tight mt-0.5 line-clamp-2">{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400">
            ...
            <span>Encrypted token authentication via Firebase & MongoDB</span>
          </div>
        </motion.div>
          ...
          <div className="rounded-2xl bg-slate-900/80 border border-white/10 p-6 sm:p-8 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.6)]">
            ...
            <div className="mb-6">
              ...
              <p className="text-xs sm:text-sm text-slate-400 mt-1">
                Continue building your developer profile.
              </p>
            </div>
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1.5" htmlFor="email-input">
                  Email Address
                </label>
